...
// +(A / B) - always okay (the resulting binary op has the highest grammatic precedence)
    ...
    public:
        ...
        virtual double Evaluate(std::function<std::variant<double, FormulaError>(Position)> func) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(std::function<std::variant<double, FormulaError>(Position)> func) const override {
                // Скопируйте ваше решение из предыдущих уроков.
                auto lhs_value = lhs_->Evaluate(func);
                auto rhs_value = rhs_->Evaluate(func);
                double result = 0.0;
                if (type_ == Add) {
        ...
        public:
            ...
            double Evaluate(std::function<std::variant<double, FormulaError>(Position)> func) const override {
                std::variant<double, FormulaError> value = func(*cell_);
...
double FormulaAST::Execute(std::function<std::variant<double, FormulaError>(Position)> func) const {
    return root_expr_->Evaluate(std::move(func));
}
